...
int medium_priority(float arr_numbers[MAX], char arr_operators[MAX], int N) {
    int i = 0;
    for (i = 0; i < N - 1; i++) {
        if (arr_operators[i] == '#') {
            arr_numbers[i] = (arr_numbers[i] + arr_numbers[i + 1]) * (arr_numbers[i] + arr_numbers[i + 1]);
            N = arr_elim(arr_numbers, arr_operators, N, i);
            i--;
        }
    }
...
// pr(arr_numbers, arr_operators, N);
    ...
    N = medium_priority(arr_numbers, arr_operators, N);
